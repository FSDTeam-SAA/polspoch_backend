...
export const calculateRebarQuote = async (req: Request, res: Response): Promise<void> => {
    try {
        const { 
            shapeName,
            diameter,   
            units,      
            sizeA, sizeB, sizeC, sizeD // in mm
        } = req.body;
        ...
        if (!config || !config.rebar || !config.rebar.labour || !courier || !truck) {
            res.status(404).json({ message: "Required database configurations are missing." });
            return;
        }

        // 2. Find Diameter specific data (Weight/M and Price/KG)
        const matData = config.rebar.materialData.find(d => d.diameter === diameter);
        if (!matData) {
        ...
        const lengthPerUnit = ((sizeA || 0) + (sizeB || 0) + (sizeC || 0) + (sizeD || 0)) / 1000;
        ...
        const labourTotal = config.rebar.labour.startingPrice + (totalWeight * config.rebar.labour.pricePerKg);
        const totalProductPrice = (materialTotal + labourTotal) * config.rebar.margin;
         const pricePerUnit=  totalProductPrice/units
        // 4. SHIPPING CALCULATION
        // Find the longest side to determine if it fits in a courier van
        const maxDimension = Math.max(sizeA || 0, sizeB || 0, sizeC || 0, sizeD || 0);
        let shippingCost = 0;
        let shippingMethod = '';
...
export const calculateBendingQuote = async (req: Request, res: Response): Promise<void> => {
    try {
        const { 
            shapeName, material, thickness, units, length, 
        ...
        const matData = config.bending.materialData.find(d => d.thickness === thickness);
        if (!matData) {
        ...
        if (!materialPricePerKg || materialPricePerKg === 0) {
            res.status(400).json({ message: `Material ${material} is not available for this thickness.` });
            return;
        }

        // 3. PRODUCT CALCULATION
        const totalWidth = (sizeA || 0) + (sizeB || 0) + (sizeC || 0) + (sizeD || 0) + (sizeE || 0) + (sizeF || 0);
        ...
        const labourTotal = config.bending.labour.startingPrice + ((numBends || 0) * units * config.bending.labour.pricePerBend);
        
        const totalProductPrice = (materialTotal + labourTotal) * config.bending.margin;
        ...
        if (maxDimension <= courier.maxSizeAllowed) {
            shippingMethod = 'courier';
            let cost = courier.basePrice;
            if (totalWeight > courier.freeWeightLimit) cost += (totalWeight - courier.freeWeightLimit) * courier.extraWeightPrice;
            ...
            if (totalWeight > truck.freeWeightLimit) {
                ...
                cost += Math.ceil(extra / truck.extraWeightStep) * truck.extraWeightPrice;
            }
            shippingCost = cost;
        }
...
export const calculateCuttingQuote = async (req: Request, res: Response): Promise<void> => {
    try {
        const { 
            shapeName, material, thickness, units, 
            sizeA, sizeB, internalCuts 
        } = req.body;
        ...
        // 1. Find Price Data
        const matData = config.cutting.materialData.find(d => d.thickness === thickness);
        if (!matData) {
            res.status(400).json({ message: `Thickness ${thickness}mm not found.` });
            return;
        }
        ...
        const labourTotal = config.cutting.labour?.startingPrice + 
                           ((internalCuts || 0) * units * config.cutting.labour?.priceInternal);
        
        // Applying Margin
        const totalProductPrice = (materialTotal + labourTotal) * config.cutting.margin;
        const pricePerUnit = totalProductPrice / units;
        ...
        if (maxDimension <= courier.maxSizeAllowed) {
            ...
            if (totalWeight > courier.freeWeightLimit) {
                calc += (totalWeight - courier.freeWeightLimit) * courier.extraWeightPrice;
            }
            if (maxDimension >= courier.sizeThreshold) {
            ...
            if (totalWeight > truck.freeWeightLimit) {
                const extra = totalWeight - truck.freeWeightLimit;
                calc += Math.ceil(extra / truck.extraWeightStep) * truck.extraWeightPrice;
            }
            shippingCost = calc;
        }
